...
;(function(){
	...
	$.fn.cleverScroll = function(settings){
		...
		// Otherwise, proceeed as normal
			...
			this.each(function(i){
				...
				// Don't go any further if element is bad
				if( this !== false ){

					// Is it in the viewport?
					if( isElementInViewport(this, options.offset) ){

						// Mark as visible
						clever[newId].elements[i].visible = true;

						// Function for entering viewport
						options.onEnterViewport($(this), i);
	...
	var beClever = function(){

		if( clever.length <= 0 ) return;
	...
	$(window).on('DOMContentLoaded load resize scroll touchend', function(){

		if( Modernizr.raf ){
			requestAnimationFrame(beClever);
		}else{
			beClever();
		}
...
(function($){
	...
	$.fn.centreVertically = function(recalc){
		...
		if( recalc ) return this;

		site.resize.after[ 'centreVertically_' + c ] = function(){

			$this.centreVertically(true);
...
(function($){
	...
	var methods = {
		init: function(options){
			...
			//	Loop through each element that the function has been attached to
			this.each(function(){
				...
				//	Merge the defaults, options and data object into the options variable
				var o = options ? $.extend({}, defaults, options, $this.data()) : $.extend({}, defaults, $this.data());

				//	If we don't have a from value, use the text from the element
				if( !o.from )
					o.from = parseFloat( $this.text() );

				//	If we don't have a to value, use the ticker data
				if( !o.to )
					o.to = o.ticker;

				//	Store the offset and other options
				tickers.ticker[id] = {
					element: $this,
					options: o,
					trigger: onload ? false : getTrigger( $this ) + getOffset(o.offset),
					zeros: o.to > o.from ? o.to.toString().length : o.from.toString().length,
					done: false
				};

				//	Set the id back onto the element for custom triggering
				$this.data('tickerid', id);

				//	Are we on load?
				if( onload )
					$this.ticker('trigger');

				//	Increase the number of tickers globally
				tickers.count++;
		...
		trigger: function(){
			...
			this.each(function(){
				...
				triggerTicker( $(this).data('tickerid') );
		...
		remove: function(){
			...
			$(window).off('scroll.ticker resize.ticker');
	...
	/**
	 *	Loops through each ticker element
	 *	and checks it's offset top against
	 *	the scroll position
	 */
	var onScroll = function(){

		//	Get the scrollTop value cross browser
		tickers.scroll = document.documentElement.scrollTop;
		tickers.scroll = tickers.scroll == 0 ? document.body.scrollTop : tickers.scroll;

		//	Loop through each ticker
		for( var i = tickers.ticker.length - 1; i >= 0; i-- ) {

			if( tickers.scroll >= tickers.ticker[i].trigger && !tickers.ticker[i].done && tickers.ticker[i].options.scroll && tickers.ticker[i].trigger ){

				//	Set things in motion!
				triggerTicker(i);

				//	Set this to done so we don't use it again
				tickers.ticker[i].done = true;
	...
	/**
	 *  Returns the trigger point for a ticker
	 *
	 *	@param {Object} $ticker
	 *	@return {String} trigger
	 */
	var getTrigger = function($ticker){

		//	Get the trigger point
		var trigger = $ticker.offset().top - $win.height();
	...
	var getOffset = function(offset){
		...
		if( typeof offset == 'string' ){
			...
			return Math.round( ( $win.height() / 100 ) * parseFloat( offset ) );
	...
	/**
	 *  Binds a scroll event to the window
	 *  but only allows one instance
	 */
	var applyOnce = function(){
		...
		if( !tickers.ready ){
			...
			$win.on({
				...
				'resize.ticker': function(){

					clearTimeout(tickers.resize);

					tickers.resize = setTimeout(function(){

						tickers.resize = false;

						//	Loop through each ticker
						for( var i = tickers.ticker.length - 1; i >= 0; i-- ) {

							tickers.ticker[i].trigger = getTrigger( $(tickers.ticker[i].element) );
	...
	/**
	 *  Wraps each character with a defined tag
	 *
	 *  @param  {[type]} number the original value
	 *  @param  {[type]} tag    the tag to wrap
	 *
	 *  @return {[type]}        the value as a string with wrapped tag
	 */
	function wrapTag(number, tag){
		...
		for (var i = 0, len = sNumber.length; i < len; i += 1) {
			...
			wrapped.push(+sNumber.charAt(i));

		};

		wrapped = '<' + tag + '>' + wrapped.join('</' + tag + '><' + tag + '>') + '</' + tag + '>';
	...
	/**
	 *	Increases a value over a duration
	 *	at a specified speed
	 */
	var triggerTicker = function(id) {

		//	Cache the ticker element
		var t = tickers.ticker[id],
			elapsedTime = 0,
			value = false;

		var loop = function(){

			//	Set an interval that will continue until the value has reached its destination
			var timer = setTimeout(function(){

				//	Work out the new value, based on a percentage of the time elapsed
				newValue = ( ( elapsedTime * ( t.options.to - t.options.from ) ) / t.options.duration ) + t.options.from;

				if( t.options.decimalplace ){

					newValue = newValue.toFixed( t.options.decimalplace );
				...
				if( value != newValue ){
					...
					//	Don't go to far!
					if( value > t.options.to )
						value = t.options.to;

					//	Add in leading zeros
					if( t.options.leadingzeros )
						value = leadingZeros(value, t.zeros);

					//	Wrap each character
					if( t.options.wrap )
						value = wrapTag(value, t.options.wrap);

					//	Does the number need commas
					if( t.options.commas )
						value = numberWithCommas(value);

					//	Do we need to append anything?
					if( t.options.append )
						value = value + t.options.append;

					//	Do we need to prepend anything?
					if( t.options.prepend )
						value = t.options.prepend + value;

					//	Update the DOM
					$(t.element).html(value);

				}

				//	Have we finished the process?
				if( elapsedTime >= t.options.duration ){

					//	Clear the interval, we are done
					clearTimeout(timer);
				...
				//	Incease the elapsed time by the speed... And repeat.
				elapsedTime += t.options.speed;
	...
	/**
	 *	A plugin function that can be attached
	 *	to any element
	 */
	...
	$.ticker = function(methodOrOptions){
...
(function($) {
  ...
  var SelectionSharer = function(options) {
    ...
    this.shareEmail = function(e) {
      ...
      $(this).attr("href","mailto:?body="+email.body);
      self.hide();
      return true;
    };
    ...
    this.render = function() {
      var popoverHTML =  '<div class="selectionSharer" id="selectionSharerPopover" style="position:absolute;">'
                       ...
                       + '    <ul>'
                       + '      <li><a class="action tweet internal" href="" title="Share this selection on Twitter" target="_blank">Tweet</a></li>'
                     //   + '      <li><a class="action facebook" href="" title="Share this selection on Facebook" target="_blank">Facebook</a></li>'
                     //   + '      <li><a class="action email internal" href="" title="Share this selection by email" target="_blank"><svg width="20" height="20"><path stroke="%23FFF" stroke-width="6" d="m16,25h82v60H16zl37,37q4,3 8,0l37-37M16,85l30-30m22,0 30,30"/></svg></a></li>'
                       + '    </ul>'
                       + '  </div>'
...
var $site = {
	...
	page: $('#page-wrapper'),
	header: $('#site-wrapper > header:first'),
	footer: $('#site-footer'),
	nav: {
		primary: $('#primary-nav'),
		menu: $('#site-menu')
...
var afterResize = function(){
	...
	site.scroll.max = getMaxBodyScroll();
...
var clearResize = function(time, name){

	delete site.resize[time][name];

};

var clearScroll = function(name){

	delete site.scroll.listener[name];

};

var getMaxScroll = function($element){
	...
	values['maxX'] = values.innerX - $element.width();
	values['maxY'] = values.innerY - $element.height();
...
;(function(){
	...
	var $header = $site.wrapper.children('header');

	if( $header.length < 1 ) return;

	$header = {
		root: $header,
		slides: $header.find('.slide'),
		active: $header.find('.active'),
		loading: {
			root: $header.find('.loading'),
			svg: $header.find('.loading svg')
		},
		nav: {
			root: $header.find('.header-navigation'),
			active: $header.find('.header-navigation .on')
		},
		arrows: $header.find('.quick-navigation')
	};

	var header = {
		active: 0,
		total: $header.slides.length,
		rootFunction: $header.root.data('function'),
		init: function(){

			TweenMax
				.to($header.loading.root, .2, {
					autoAlpha: .2
				});

			TweenMax
				.from($header.loading.svg.find('rect'), 1, {
					y: '100%',
					delay: .5,
			...
			var activeFunction = $header.active.data('function');

			if( activeFunction ){
				header.functions[ activeFunction ]($header.active, true);
				header.functions[ activeFunction ]($header.active);
			}
			...
			// Add nav class if required
			$site.nav.primary
				.toggleClass('light', $header.active.data().nav == 'light');

			if( $header.arrows.length >= 1 ){

				$header.arrows
					.on('click', this, function(e){

						var num = header.active;

						if( $(this).is('.quick-navigation--prev') ){
							num--;
						}else{
							num++;
						}

						if( num > header.total - 1 ){
							num = 0;
						}else if( num < 0 ){
							num = header.total - 1;
						}
			...
			// Animate in
			TweenMax
				.to([$header.active, $header.nav.root], .75, {
					opacity: 1,
					ease: Cubic.easeInOut,
					force3D: true,
					delay: 1.5
			...
			if( site.width >= setting.mq.small ){

				TweenMax
					.to($site.nav.primary, 1, {
						y: '0%',
						autoAlpha: 1,
						force3D: true,
						delay: 1.5,
		...
		goTo: function(num, skipAnimation){

			var $newSlide = $( $header.slides[num] ),
				$newSlideVideo = {
					root: $newSlide.find('.slide-video'),
					video: $newSlide.find('.slide-video').get(0)
				},
				data = $newSlide.data(),
				hasFunction = data.function && header.functions[ data.function ],
				left = num > header.active;
			...
			TweenMax
				.to($newSlide, skipAnimation ? 0 : .4, {
					...
					onComplete: function(){
						...
						// Run function
						if( hasFunction )
							header.functions[ data.function ]($newSlide);
			...
			// Run function
			if( hasFunction )
				header.functions[ data.function ]($newSlide, true);

			// Add nav class if required
			$site.nav.primary
				.toggleClass('light', data.nav == 'light');

			// Stop any videos
			if( $header.active.find('.slide-video').length ){
				$header.active.find('.slide-video').get(0).pause();
			}

			// Store the new slide
			$header.active.removeClass('active');
			$header.active = $newSlide;
			$header.active.addClass('active');

			// Swap nav states
			$header.nav.active.removeClass('on');
			$header.nav.active = $header.nav.root.find('li:eq(' + num + ')');
			$header.nav.active.addClass('on');

			// Play any videos
			if( $newSlideVideo.root.length ){
				// $header.active.find('.slide-video').get(0).load();
				// $header.active.find('.slide-video').get(0).play();
				$newSlideVideo.video.load();
				$newSlideVideo.video.play();
			}
		...
		createVideos: function(){
			...
			$header.slides
				.each(function(){
					...
					if( !$slide.root.data('video') ){
						return;
					}

					var videoAttributes = $slide.active ? 'autoplay style="opacity: 0;"' : '',
						videoHTML = '<video class="slide-video" ' + videoAttributes + ' loop autoplay preload muted poster="' + $slide.root.data('video-poster') + '">'
		        			+ '<source src="' + $slide.root.data('video') + '" type="video/mp4"></source>'
		    				+ '</video>';

		    		if( Modernizr.video && $core.body.width() >= setting.mq.medium ){

		    			if( $slide.root.find('.slide-video').length ){
		    				return;
		    			}

		    			$slide.root
		    				.css({
		    					'background-image': 'url(' + $slide.root.data('video-poster') + ')'
		    				})
		    				.find('.slide-inner').append(videoHTML);

						if( $slide.active ){

							TweenMax
								.to($slide.root.find('.slide-video'), 2, {
									opacity: 1,
									ease: Cubic.easeInOut
								});
	...
	if( $header.root.is('.header-slider') ){

		header.init();

		$header.nav.root
			.on('click', 'li:not(.on)', function(e){

				header.goTo( $(this).index() );

				e.preventDefault();

			});

	}else if( header.rootFunction && header.functions[ header.rootFunction ] ){

		header.functions[ header.rootFunction ]();
		...
		if( $header.root.is('.work-header') ){

			var $content = $header.root.find('.header-content'),
				$client = $content.find('p.client-name'),
			...
			TweenMax
				.staggerFromTo(titleSplit.chars, 4, {
					opacity: 0
				},{
					opacity: 1,
					force3D: true,
	...
	site.resize.after['createSliderVideos'] = function(){
...
;(function(){
	...
	var $tm = $site.footer.children('#footer-strapline'),
		$btt = $site.footer.children('#back-to-top');

	var strapline = new SplitText($tm, {
		type: 'chars'
	});
	...
	$site.footer
		.cleverScroll({
			...
			onEnterViewport: function(){
				...
				TweenMax
					.staggerFromTo(strapline.chars, 1.5, {
						opacity: 0
					},{
						opacity: 1,
						ease: Cubic.easeIn
	...
	site.resize.after['footerPush'] = function(){

		var header = $site.header.length ? $site.header.height() : 0;

		TweenMax
			.set($site.page, {
				minHeight: site.height - $site.footer.outerHeight(true) - header
			});

	};

	site.resize.after.footerPush();
	...
	$('[data-animate]')
		.cleverScroll({
			ready: function($el){
				...
				var animationType = $el.data('animate');
			...
			onEnterViewport: function($el){

				var animationType = $el.data('animate'),
					animationDuration = $el.data('animate-duration') || .6,
				...
				switch( animationType ){
					...
					case 'sequential-fade':
						...
						TweenMax
							.staggerFromTo($el.children(), animationDuration, {
								opacity: 0
							},{
								opacity: 1,
								ease: Expo.easeOut
...
;(function(){
	...
	function getTriggers(){
		...
		return {
			hide: header ? $site.header.innerHeight() - height : 0,
			fix: header ? height * 1.5 : $site.nav.primary.offset().top
		}

	};

	function getHeight(){
		return $site.nav.primary.outerHeight();
	}

	var active = site.width >= setting.mq.small,
	...
	var toggleFixed = function(toggle){

		$site.nav.primary
			.toggleClass('fixed', toggle);

		TweenMax
			.set($site.nav.primary, {
				y: toggle ? '-100%' : '0%'
			});
	...
	var toggleVisibility = function(toggle){
		...
		TweenMax
			.to($site.nav.primary, toggle ? .6 : .3, {
				y: toggle ? '0%' : '-100%',
				ease: toggle ? Expo.easeOut : Cubic.easeIn,
				force3D: true
			});
	...
	var scrollHandler = function(){
		...
		// Up or down
		up = scroll > site.scroll.y;

		// To prevent weirdness when dealing with elastic overscrolling on OS X
		if( site.scroll.y < 0 || site.scroll.y > site.scroll.max || site.nav.menu ) return;

		// Store the distance
		if( site.nav.fixed && site.scroll.y > t.hide ){

			if( !d.up && up && !site.nav.visible )
				d.up = site.scroll.y;

			if( !d.down && !up && site.nav.visible )
				d.down = site.scroll.y;

		}

		// Hide it instantly
		if( site.nav.visible && site.nav.fixed && up && site.scroll.y <= t.hide && t.hide ){
			...
			site.nav.visible = false;

		}

		// To fix or not to fix, that... Is decided right now by this if statement
		if(
			( site.nav.fixed && site.scroll.y <= t.fix ) ||
			( !site.nav.fixed && site.scroll.y > t.fix + ( header ? 0 : height ) )
		){
			...
			d.down = d.up = false;
			site.nav.fixed = !site.nav.fixed;

			toggleFixed(site.nav.fixed);

		}

		if(
			( site.nav.fixed && up && !site.nav.visible && d.up - site.scroll.y > d.trigger && site.scroll.y > t.fix ) ||
			( site.nav.fixed && !up && site.nav.visible && site.scroll.y - d.down > d.trigger )
		){

			d.up = d.down = false;
			site.nav.visible = !site.nav.visible;

			toggleVisibility(site.nav.visible);

		}

		scroll = site.scroll.y;

	};

	// Add function to the global scroll listener
	if( active )
		site.scroll.listener['primaryNav'] = scrollHandler;

	// Turn it on or off on resize
	site.resize.after['primaryNav'] = function(){

		var nowActive = site.width >= setting.mq.small;

		if( active && !nowActive ){

			active = false;

			clearScroll('primaryNav');
	...
	if( header )
		$site.nav.primary.addClass('has-header');
...
;(function(){
	...
	var $menu = {
		root: $site.nav.menu,
		wrapper: $site.nav.menu.find('#site-menu-wrapper'),
		dim: $site.nav.menu.find('#site-menu-dim'),
		blog: {
			root: $site.nav.menu.find('#site-menu-blog'),
			inner: $site.nav.menu.find('#site-menu-blog-inner')
		},
		nav: {
			root: $site.nav.menu.find('#site-menu-navigation'),
			inner: $site.nav.menu.find('#site-menu-navigation-inner')
		},
		open: $('#site-menu-open'),
		close: $site.nav.menu.find('.site-menu__close')
	};
	...
	var menu = {
		...
		getTop: function($el){
			...
			var wH = $el.root.innerHeight(),
				iH = $el.inner.innerHeight(),
				top = ( wH - iH ) / 2;
		...
		layout: function(){
			...
			menu.blog = site.width >= setting.mq.small;

			// Vertical layout
			TweenMax
				.set([$menu.blog.inner, $menu.nav.inner], {
					clearProps: 'top'
				});

			var navTop = menu.getTop($menu.nav);

			if( menu.blog ){

				var blogTop = menu.getTop($menu.blog);

				TweenMax
					.set([$menu.blog.inner, $menu.nav.inner], {
						top: navTop < blogTop ? navTop : blogTop
					});
		...
		open: function(e){
			...
			TweenMax.set($menu.nav.root, { y: '-100%' });
			TweenMax.set($menu.blog.root, { y: '100%' });

			TweenMax
				.to([$menu.nav.root, $menu.blog.root], menu.speed, {
					y: '0%',
					force3D: true,
					ease: Cubic.easeInOut
				});

			TweenMax.set($menu.nav.inner, { y: '-20%' });
			TweenMax.set($menu.blog.inner, { y: '20%' });

			TweenMax
				.fromTo([$menu.nav.inner, $menu.blog.inner], menu.speed * 2, {
					opacity: 0
				},{
					opacity: 1,
					y: '0%',
	...
	$menu.close.on('click', menu.close);
	$menu.open.on('click', menu.open);

	site.resize.after['menu'] = menu.layout;
...
;(function(){
	...
	$('.scrolling-screenshot')
		.each(function(i){
			...
			var $this = $(this),
				$wrap = $this.find('.scrolling-screenshot__img-wrapper'),
				$img = $wrap.children('img'),
				id = 'screenshot-' + i,
				rootHeight = wrapHeight = imgHeight = startY = endY = max = imgHeight = 0;
				...
				var layout = function(){
					...
					startY = $wrap.offset().top + ( wrapHeight / 2 ) - site.height;
					endY = startY + site.height;
					max = imgHeight - wrapHeight;
				...
				site.scroll.listener[id] = function(){

					if( site.scroll.y < startY || site.scroll.y > endY ) return;

					var prog = rangeToRange(site.scroll.y, endY, startY, 1, 0);
					...
					timeline.progress(prog);

				};

				site.resize.after[id] = layout;
...
;(function(){
	...
	var count = 0,
		...
		slider = [],
		defaults = {
			startAt: 0, // An alternative index
			auto: false, // Scroll without input?
			nav: '.full-width-slider-nav', // Can specify a selector relative to the root
			arrows: false,
			ease: Cubic.easeInOut,
			speed: .4,
			draggable: true
	...
	var methods = {
		init: function(el, i, settings){
			...
			var $slider = $(el),
				...
				data = $slider.data(),
				id = total + i,
				options = $.extend({}, settings, data);

			// Store this instance
			var s = {
				root: $slider,
				wrapper: $slider.children('.slide-wrapper'),
				slides: $slides,
				width: $slider.width(),
				nav: options.nav ? $slider.next(options.nav) : false,
				arrows: options.arrows ? $slider.siblings(options.arrows) : false,
				current: options.startAt ? options.startAt : 0,
				total: $slides.length,
				options: options
			};
			...
			// Position a few things
			methods.position(id);

			// Apply an initial layout
			methods.setHeight(id);

			// Go looking for images, load them, callback
			methods.loadImages(id, s.current, function(){
				...
				TweenMax
					.set(slider[id].slides[slider[id].current], {
						visibility: 'visible'
					});
			...
			if( options.arrows ){
				...
				s.arrows
					.on('click', 'a', function(e){
						...
						if( $(this).is('.quick-navigation__prev') ){
							num--;
						}else{
							num++;
						}
			...
			if( options.draggable ){
				...
				s.bounds = $('<span/>', {
						css: {
							position: 'absolute',
							top: 20,
							left: 0 - ( 100 * ( s.total - 1 ) ) + '%',
							width: ( 200 * s.total ) - 100 + '%',
							height: 10
						}
					})
					.appendTo(s.root);
					...
					.create(s.wrapper, {
						...
						snap: {
							x: function(x){
								...
								var newX = Math.round( x / s.width ),
									num = Math.abs(newX);

								if( num >= s.total ){
									methods.goTo(id, s.total - 1, 'drag');
								}else if( newX >= 0 ){
									methods.goTo(id, 0, 'drag');
								}else{
									methods.goTo(id, num, 'drag');
								}
		...
		setHeight: function(id){
			...
			TweenMax
				.set(s.root, {
					height: s.slides.first().innerHeight()
				});
		...
		setNav: function(id){
			...
			s.nav
				.find('li:eq(' + s.current + ')')
				.addClass('on')
				.siblings('.on')
				.removeClass('on');

		},
		loadImages: function(id, img, callback){

			var s = slider[id],
				$slide = $(s.slides[img]),
				$images = $slide.find('img, [data-img], [data-image]'),
				total = $images.length,
				callback = callback ? callback : function(){};

			if( $slide.data('loaded') || total === 0 && callback ){
		...
		goTo: function(id, num, ev) {

			var s = slider[id],
				$slide = $(s.slides[num]),
				x = 0 - ( s.width * num );

			// Do we need to load imaages
			if( !$slide.data('loaded') ){

				methods.loadImages(id, num, function(){

					$slide.data('loaded', true);

					methods.loading(id, num);
			...
			// Alter the height, but don't do it nicely, things should be the same size ideally
			TweenMax
				.set(s.root, {
					height: h
				});
		...
		loading: function(id, num, toggle){
		...
		position: function(id){
			...
			// Work out the slide width
			var width = 100 / s.total;

			// Apply dimensions to a few things
			TweenMax
				.set(s.wrapper, {
					width: ( 100 * s.total ) + '%',
					x: 0 - ( s.current * width ) + '%'
		...
		layout: function(){
			...
			$.each(slider, function(i){
				...
				s.width = s.root.width();

				methods.goTo(i, s.current);
	...
	$.fn.fullWidthSlider = function(settings){
		...
		this.each(function(i){
			methods.init(this, i, options);
		});

		total = total + count;

		site.resize.after['fws-' + total + count] = function(){
	...
	$('.full-width-slider').fullWidthSlider();

}());

;(function(){

	var $news = $('#news-grid-wrapper');

	if( $news.length < 1 ) return;

	var $news = {
		root: $news,
		wrapper: $('#news-grid-items'),
		items: $('#news-grid-items').find('.news-grid-item'),
		scrollbar: {
			root: $('#news-grid-scrollbar'),
			handle: $('#news-grid-scrollbar').children()
		},
		video: {
			root: $('#news-video'),
			wrapper: $('#news-video-wrapper')
		}
	};
	...
	var news = {
		...
		init: function(){
			...
			if( site.width > setting.mq.small && !Modernizr.touch && !news.active ){
				news.enable();
			}else if( site.width <= setting.mq.small && news.active ){
				news.disable();
			}else{
				news.checkVideo();
			}
		...
		enable: function(){
			...
			TweenLite
				.set($news.wrapper, {
					width: ( 33 * $news.items.length ) + 10 + 'rem'
				});

			if( news.ready ){

				news.items[0].enable();
				news.handle[0].enable();

			}else{

				news.ready = true;

				news.items = Draggable
					.create(
						$news.wrapper,
						$.extend(
							{},
							...
							{
								bounds: $news.root,
								onDrag: news.updateHandle,
								onThrowUpdate: news.updateHandle,
				...
				news.handle = Draggable
					.create(
						...
						$.extend(
							...
							news.options,
							{
								bounds: $news.scrollbar.root,
								onDrag: news.updateItems,
								onThrowUpdate: news.updateItems
							}
						)
					);
		...
		disable: function(){
			...
			news.items[0].disable();
			news.handle[0].disable();

			TweenMax
				.set([$news.scrollbar.handle, $news.wrapper], {
					clearProps: 'all'
				});
		...
		updateHandle: function(){
			...
			TweenLite
				.set($news.scrollbar.handle[0], {
					x: rangeToRange( this.x, news.items[0].minX, 0, news.handle[0].maxX, 0 )
				});
		...
		updateItems: function(){
			...
			TweenLite
				.set($news.wrapper[0], {
					x: rangeToRange( this.x, news.handle[0].maxX, 0, news.items[0].minX, 0 )
				});

		},
		checkVideo: function(){

			var width = $news.video.wrapper.width(),
				height = $news.video.wrapper.height();

			var widthScale = width / news.video.width,
				heightScale = height / news.video.height;

			$news.video.root
				.toggleClass('fill-height', widthScale < heightScale);

		}
	};

	site.resize.after['newsSlider'] = function(){

		if( site.old.width == site.width ) return;
...
;(function(){
	...
	var $workPanels = $('.work-panel-grid');

	if( $workPanels.length < 1 ) return;

	var $workPanels = {
		root: $workPanels,
		rows: $workPanels.find('> .row'),
		panels: $workPanels.find('.work-panel')
	};
	...
	var workPanels = {
		init: function(){
			...
			}else if( site.width < setting.mq.small ){

				workPanels.enableAll();

			}

			workPanels.setBackgrounds();
		...
		enable: function(){
			...
			$workPanels.rows
				.each(function(){
					...
					$columns
						.find('.work-panel, .work-panel a.inner')
							.css('height', 'auto');
		...
		enableAll: function(){
			...
			var $panels = $('.work-panel, .work-panel a.inner'),
				maxHeight = 0;
		...
		setBackgrounds: function(){
			...
			$workPanels.panels
				.each(function(){
					...
					var $this = $(this);
						data = $this.data()
						backgroundImage = (site.width >= setting.mq.large) ? data.bgDesktop : (site.width >= setting.mq.small) ? data.bgTablet : data.bgMobile;
	...
	site.resize.after['workPanels'] = function(){
	...
	$('.stats-list')
		.cleverScroll({
			...
			onEnterViewport: function($el){
				...
				TweenMax
					.staggerFromTo($el.children('li'), 1, {
						y: '20%'
					},{
						y: '0%',
						opacity: 1,
				...
				TweenMax
					.staggerFromTo($el.children('li').find('img'), .6, {
						scale: .5
					},{
						scale: 1,
						delay: .5,
				...
				TweenMax
					.staggerFromTo($el.children('li').find('img'), .4, {
						opacity: 0
					},{
						opacity: 1,
						delay: .5
...
$site.bottle = {
	root: $('#primary-header'),
	inner: $('#primary-header').children('.inner'),
	arrow: $('#primary-header').find('.arrow')
};

$bottle = {
	root: $bottle,
	canvas: $bottle.children('canvas'),
	roundel: $bottle.children('h1'),
	loading: {
		root: $('#loading-bar'),
		bar: $('#loading-bar').children('span')
	}
};
...
var bottle = {
	...
	rotation: 0,
	mask: '/assets/img/work/beer/bottle-assets/bottle/mask.png',
	path: '/assets/img/work/beer/bottle-assets/bottle/{num}.jpg',
	process: 'http://engageinteractive.co.uk/assets/global/php/image.php?src={path}&w=120&h=372&zc=1&q=90',
	images: [],
	context: $bottle.canvas[0].getContext('2d'),
	size: {
		max: 78, // Percent
		width: $bottle.canvas.prop('width'),
		height: $bottle.canvas.prop('height'),
		scaleWidth: false,
		scaleHeight: false
	},
	spin: {
...
var stageBottle = function(fresh){

	var stage = $site.bottle.root.height(),
		mobile = site.width < setting.mq.mobile;

	var height = ( stage / 100 ) * ( mobile ? bottle.size.max / 1.2 : bottle.size.max ),
		scale = ( height / bottle.size.height ),
		width = scale * bottle.size.width;
	...
	bottle.size.scaleWidth = width;
	bottle.size.scaleHeight = height;
...
var loadImages = function(callback){
	...
	// First, load in the mask
	var mask = new Image();
	mask.src = bottle.mask;
	...
	$.preload({
		...
		ready: function(){
			...
			// Apply the mask
			bottle.context.drawImage(bottle.mask, 0, 0);
			bottle.context.globalCompositeOperation = 'source-in';

			// Load in the remaining images
			var loaded = 0;

			for( var i = 0; i <= bottle.frame.total - 1; i++ ){

				bottle.images[i] = new Image();

				var src = bottle.path.replace('{num}', pad(i, 2));

				if( site.height < 520 )
					src = bottle.process.replace('{path}', src);

				$.preload({
					src: src,
					ready: function(){

						if( ++loaded >= bottle.frame.total ){

							callback();

							TweenMax
								.to($bottle.loading.root, .2, {
									opacity: 0,
									onComplete: function(){
						...
						TweenMax
							.set($bottle.loading.bar, {
								width: rangeToRange(loaded, bottle.frame.total, 0, 100, 0) + '%'
							});

					}
				});

				bottle.images[i].src = src;
...
var updateFrame = function(){
	...
	var rotation = bottle.rotation - ( Math.floor( bottle.rotation / 360 ) * 360 ),
		newFrame = Math.round( rangeToRange(rotation, 360, 0, bottle.frame.total - 1, 0) );

	// Only render a new frame if we need to
	if( bottle.frame.current != newFrame ){
		bottle.frame.current = newFrame;
		bottle.context.drawImage(bottle.images[bottle.frame.current], 0, 0, 226, 702);
	}

};

var spinTheBottle = function(){

	$bottle.canvas
		.on('mouseenter touchstart', function(e){

			clearTimeout(bottle.idleTimer);

			bottle.mouseEntered = true;

			bottle.spin.start = {
				mouse: Modernizr.touch ? e.originalEvent.touches[0].pageX : e.pageX,
				rotation: bottle.rotation
			};

			// Make sure to clear any existing velocities
			bottle.spin.velocity = ThrowPropsPlugin.track(bottle, 'rotation');
		...
		.on('mousemove touchmove', function(e){
			...
			if( !bottle.mouseEntered || !bottle.spin.velocity ) return false;

			var dist = ( Modernizr.touch ? e.originalEvent.touches[0].pageX : e.pageX ) - bottle.spin.start.mouse;

			TweenMax
				.set(bottle, {
					rotation: bottle.spin.start.rotation - dist
				});
		...
		.on('mouseleave touchend', function(e){
			...
			ThrowPropsPlugin
				.to(bottle, {
					throwProps: {
						rotation: {
							resistance: Modernizr.touch ? 20 : 10,
							velocity: ThrowPropsPlugin.getVelocity(bottle, 'rotation')
						}
					},
					ease: Power3.easeOut,
					onUpdate: updateFrame,
					onComplete: function(){
						...
						if( !bottle.mouseEntered ){
							ThrowPropsPlugin.untrack(bottle, 'rotation');
							bottle.spin.velocity = false;
						}

						bottle.idleTimer = setTimeout(reset, 3000);
...
$bottle.roundel
	.on('click', function(e){
		...
		if( bottle.dave > 40 ) return false;

		bottle.dave++;

		var $dave = $('<span class="dave"></span>').appendTo($site.bottle.inner),
			scale = randInRange(.5, 1.5),
			y = randInRange( 100 - ( site.height / 3 ), site.height / 2.5 );

		$dave.addClass('dave-' + Math.round(randInRange(1,4)));

		TweenMax
			.set($dave, {
				x: -200 - ( site.width / 2 ),
				y: y,
				zIndex: scale > 1.4 ? 3 : 0,
				rotation: 10,
				scale: scale
			});
...
site.resize.after['bottle'] = stageBottle;

site.resize.before['bottle'] = function(){
...
loadImages(function(){
	...
	TweenMax
		.to(bottle, 3, {
			...
			ease: Power2.easeOut,
			onUpdate: updateFrame,
			onComplete: spinTheBottle
		});

	// Automatically show the roundel alt state on touch devices
	if( Modernizr.touch ){

		setInterval(function(){

			$bottle.roundel
				.toggleClass('click-and-tap');
...
// KONAMI!!!!!
var domdomdom = new Konami(function(){

	$core.body
		.addClass('dom');

	$('#quinlan')
		.html("RARE! The only bar in town where you'll be served drink by Kenickie from the hit 1978 film Greace.");
...
;(function(){
	...
	if ($mapCanvas.length) {
		...
		function initialize()
		{
			var mapCenter = new google.maps.LatLng(53.797376, -1.534393),
				parking1 = new google.maps.LatLng(53.798912, -1.532769),
				parking2 = new google.maps.LatLng(53.797237, -1.531830),
				parking3 = new google.maps.LatLng(53.796483, -1.536953),
				engage = new google.maps.LatLng(53.79706, -1.53413),
				initialZoom = 16;

			var mapStyles = [{"featureType":"administrative","elementType":"labels.text.fill","stylers":[{"color":"#444444"}]},{"featureType":"administrative.neighborhood","elementType":"geometry","stylers":[{"visibility":"off"},{"saturation":"0"}]},{"featureType":"landscape","elementType":"all","stylers":[{"color":"#f2f2f2"}]},{"featureType":"landscape.man_made","elementType":"all","stylers":[{"visibility":"on"}]},{"featureType":"landscape.natural.terrain","elementType":"geometry","stylers":[{"visibility":"off"}]},{"featureType":"poi","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"road","elementType":"all","stylers":[{"visibility":"simplified"},{"saturation":"-100"}]},{"featureType":"road","elementType":"geometry","stylers":[{"visibility":"simplified"},{"saturation":"0"},{"weight":"3.34"}]},{"featureType":"road","elementType":"geometry.fill","stylers":[{"lightness":"56"}]},{"featureType":"road","elementType":"geometry.stroke","stylers":[{"lightness":"-50"}]},{"featureType":"road","elementType":"labels","stylers":[{"visibility":"simplified"}]},{"featureType":"road","elementType":"labels.text","stylers":[{"weight":"3.89"}]},{"featureType":"road","elementType":"labels.text.fill","stylers":[{"weight":"5.69"}]},{"featureType":"road","elementType":"labels.text.stroke","stylers":[{"weight":"4.47"}]},{"featureType":"road","elementType":"labels.icon","stylers":[{"visibility":"off"}]},{"featureType":"road.highway","elementType":"all","stylers":[{"visibility":"simplified"},{"lightness":"-13"}]},{"featureType":"road.highway","elementType":"labels.text","stylers":[{"lightness":"-39"},{"weight":"0.01"},{"visibility":"on"}]},{"featureType":"road.highway","elementType":"labels.text.fill","stylers":[{"weight":"5.69"}]},{"featureType":"road.highway","elementType":"labels.text.stroke","stylers":[{"weight":"2.71"},{"color":"#ffffff"}]},{"featureType":"road.arterial","elementType":"labels.icon","stylers":[{"visibility":"off"}]},{"featureType":"transit","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"transit.station.bus","elementType":"all","stylers":[{"visibility":"on"},{"lightness":"-70"}]},{"featureType":"transit.station.bus","elementType":"geometry","stylers":[{"lightness":"-53"}]},{"featureType":"water","elementType":"all","stylers":[{"color":"#ffee00"},{"visibility":"on"}]}];
			...
			var iconParking = new google.maps.MarkerImage(
				'/assets/img/contact/map/icon-parking.png',
				null,
				null,
				null,
				new google.maps.Size(60, 66) // Image is retina ready so scale it down
			);

			var icon = new google.maps.MarkerImage(
				'/assets/img/contact/map/icon.png',
				null,
				null,
				null,
				new google.maps.Size(60, 66) // Image is retina ready so scale it down
			...
			google.maps.event.addListenerOnce(map, 'idle', function(){
				$locationDetails.addClass('loaded');
			});

			google.maps.event.addListener(map, 'zoom_changed', function(){
				if (map.getZoom() != initialZoom) {
					map.setCenter(engage);
					initialZoom = false;
				}
			});

			site.resize.after.map = function(){
...
;(function(){
	...
	var culture = {
		...
		messages: [
			...
			//'Here\'s some more stuff we found!',
			//'Look what else we dug up!'
		],
		layout: function($panels){

			if( ( site.width >= 560 && !culture.packeryActive ) || $panels ) {

				if( $panels ){

					$culture.packery('appended', $panels);

				}else{

					$culture
						.packery({
							itemSelector: '.culture__panel',
							stamp: '.culture__panel--sticky',
				...
				culture.packeryActive = true;

			}else if( site.width < 560 && culture.packeryActive ){

				$culture.packery('destroy');
				culture.packeryActive = false;
		...
		init: function($panels){
			...
			$panels
				.cleverScroll({
					...
					onEnterViewport: function($el){
						...
						if( culture.animating > 10 )
							culture.animating = 10;
			...
			$panels
				...
				.cleverScroll({
					...
					onEnterViewport: function($el){
						...
						$.preload({
							src: $el.data('culture-img'),
							ready: function($img, src){
			...
			$panels
				...
				.each(function(){
					...
					var $vid = $(this),
						$wrapper = $vid.parent(),
						$play = $vid.siblings('.play'),
						$mute = $vid.siblings('.audio');

					var muted = true,
						playing = false;

					var togglePlayVideo = function(toggle){

						$wrapper.toggleClass('is-playing', toggle);
						$vid[0][ toggle ? 'play' : 'pause' ]();
						playing = toggle;
					...
					$mute
						.on('click', function(){
							...
							muted = !muted;
							$vid[0].muted = muted;
							$wrapper.toggleClass('has-audio', !muted);
	...
	$culture
		.on('click', '.culture-load-more', function(e){
			...
			TweenMax
				.fromTo($msg, .2, {
					...
					onComplete: function(){
						...
						$.ajax({
							method: "GET",
							url: "/culture/page/" + page
						})
						.done(function( response ) {

							var $panels = $(response).filter('.culture__panel');

							$culture.append($panels);

							culture.layout($panels);
							culture.init($panels);

							// TODO: Use split text and do this properly
							$msg.html( culture.messages[getRandomInt(0, culture.messages.length -  1)] );
							page++;
	...
	// Initialise the functionality for various panels
	culture.init( $culture.find('.culture__panel') );

	// Lay everything out
	culture.layout();

	// Do the layout on resize
	site.resize.after['culture'] = culture.layout;
...
;(function(){
	...
	if ($post.length > 0) {
		...
		disqus_url = $post.data('disqus_url');
		disqus_title = $post.find('h1').text();
	...
	//$('.copy h1','.copy h2','.copy h3','.copy h4','.copy h5','.copy p','.copy li','.copy blockquote',).selectionSharer();
	$('.blog-content *').selectionSharer();
...
// @codekit-prepend "libs/TweenMax.js"
// @codekit-prepend "libs/packery.pkgd.js"

// GSAP plugins
// @codekit-prepend "plugins/gsap-ScrollToPlugin.js"
// @codekit-prepend "plugins/gsap-ThrowPropsPlugin.js"

// Utils
// @codekit-prepend "utils/gsap-Draggable.js"
// @codekit-prepend "utils/gsap-SplitText.js"
// @codekit-prepend "utils/scroll-scope.js"

// Plugins
// @codekit-prepend "plugins/jquery-simpleselect.js"
// @codekit-prepend "plugins/jquery-cleverScroll.js"
// @codekit-prepend "plugins/jquery-preload.js"
// @codekit-prepend "plugins/jquery-loadimg.js"
// @codekit-prepend "plugins/jquery-centreVertically.js"
// @codekit-prepend "plugins/rainbow-custom.min.js"
// @codekit-prepend "plugins/jquery.ticker.js"
// @codekit-prepend "plugins/konami.js":
// @codekit-prepend "plugins/selection-sharer.js":


// Core functionality
// @codekit-prepend "core/global.js"
// @codekit-prepend "core/header.js"
// @codekit-prepend "core/footer.js"
// @codekit-prepend "core/animate.js"
// @codekit-prepend "core/buttons.js"

// Modules
// @codekit-prepend "modules/primary-nav.js"
// @codekit-prepend "modules/site-menu.js"
// @codekit-prepend "modules/scrolling-screenshot.js"
// @codekit-prepend "modules/full-width-slider.js"
// @codekit-prepend "modules/news-slider.js"
// @codekit-prepend "modules/work-panels.js"
// @codekit-prepend "modules/stats.js"
// @codekit-prepend "modules/bottle.js"


// Pages
// @codekit-prepend "pages/contact.js"
// @codekit-prepend "pages/culture.js"
// @codekit-prepend "pages/blog.js"
